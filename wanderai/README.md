...
WanderAI is a cross-platform Expo React Native travel assistant with real-time translation, AI-ranked local discovery, itinerary planning, community reviews, utility widgets, and a freemium upgrade path.
...
```bash
npm run build --workspace @wanderai/shared
```

6. Start the backend:

```bash
npm run dev --workspace @wanderai/backend
```

7. Start Expo:

```bash
npm run start --workspace @wanderai/mobile
```
...
Creates or updates the Clerk-linked WanderAI profile.
...
```bash
docker build -f apps/backend/Dockerfile -t wanderai-backend .
docker run --env-file apps/backend/.env -p 4000:4000 wanderai-backend
```
